...
async def main():
    async with async_playwright() as p:
        ...
        for w, h in [(1440, 900), (768, 1024), (390, 844)]:
            ...
            await pg.add_style_tag(content="html{scroll-behavior:auto!important}"); await pg.wait_for_timeout(1800)
            H = await pg.evaluate("document.documentElement.scrollHeight")
            n = 0; y = 0
            while y < H and n < 25:
                await pg.evaluate(f"window.scrollTo(0,{y})"); await pg.wait_for_timeout(1400)
