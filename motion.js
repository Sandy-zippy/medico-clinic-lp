...
(function () {
  ...
  // 3. Count-up with ease-out-expo, 1.6 s. Starts at 40% so it never reads "0+".
  function count(el) {
    if (el.dataset.done) return; el.dataset.done = 1;
    const to = +el.dataset.count, from = Math.round(to * 0.4), t0 = performance.now(), D = 1600;
    const step = t => {
  ...
  // 4. Pointer tilt on photo stacks and portfolio tiles. Desktop pointers only; max 6 degrees.
  if (matchMedia("(hover: hover) and (pointer: fine)").matches) {
    document.querySelectorAll("[data-tilt], .pf figure, .stage").forEach(el => {
      const max = el.matches(".stage") ? 3 : 6;
